...
import { EMAIL_PROVIDERS } from 'app/shared';
...
export class Trie {
  ...
  private init() {
    EMAIL_PROVIDERS.forEach(email => this.add(email));
  }
  ...
  public getNode(node, word, idx) {
    if (idx === word.length - 1) {
      return node.children[word.charAt(idx)];
    } else {
      return this.getNode(node.children[word.charAt(idx)], word, ++idx);
    }
  }
